...
public class PeerChainValidator {
  ...
  private PeerChainValidator(
    ...
    Preconditions.checkNotNull(chainDataClient);
    Preconditions.checkNotNull(requiredCheckpoint);
  ...
  public SafeFuture<Boolean> validate(final Eth2Peer peer, final PeerStatus newStatus) {
    ...
    //        .exceptionally(
    //            err -> {
    //              LOG.debug("Unable to validate peer's chain, disconnecting {}", peer.getId(),
    // err);
  ...
  //      // consistency
  //      LOG.trace(
  //          "Request required checkpoint block from peer {}: {}", peer.getId(),
  // checkpointToVerify);
  ...
  //          // canonical.
  //          // So, double-check by requesting the block by slot to make sure the peer considers
  // this
  //          // block canonical.
  //          .thenCompose(
  //              maybeBlock ->
  ...
  //      LOG.debug(
  //          "Peer is advertising invalid finalized epoch {} which is at or ahead of our current
  // epoch {}: {}",
  ...
  //      LOG.trace(
  //          "Finalized epoch for peer {} matches our own finalized epoch {}, verify blocks roots
  // match",
  //          peer.getId(),
  //          localFinalizedEpoch);
  ...
  //      LOG.trace(
  //          "Our finalized epoch {} is ahead of our peer's ({}) finalized epoch {}, check that we
  // consider our peer's finalized block to be canonical.",
  //          localFinalizedEpoch,
  //          peer.getId(),
  //          remoteFinalizedEpoch);
  ...
  //      LOG.trace(
  //          "Our finalized epoch {} is behind of our peer's ({}) finalized epoch {}, check that
  // our peer considers our latest finalized block to be canonical.",
  //          localFinalizedEpoch,
  //          peer.getId(),
  //          remoteFinalizedEpoch);
  ...
  //        // Remote finalized epoch is invalid if is from the current epoch (unless we're at
  // genesis)
  ...
  //            maybeBlock ->
  //                maybeBlock
  //                    .map(block -> validateBlockRootsMatch(peer, block,
  // status.getFinalizedRoot()))
  //                    .orElseGet(
  //                        () -> {
  //                          LOG.trace(
  //                              "Missing finalized historical block corresponding to peer's latest
  // finalized checkpoint.  Unable to validate, so drop peer connection for now.");
  //                          return false;
  //                        }));
  //  }
  //
  ...
  //    } else {
  //      LOG.warn(
  //          "Detected peer with inconsistent finalized block at slot {} for peer {}.  Block roots
  // {} and {} do not match",
  //          block.getSlot(),
  //          peer.getId(),
  //          blockRoot,
